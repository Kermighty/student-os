import { CalendarDays, GraduationCap, NotebookPen, Plus } from "lucide-react";
...
export function DashboardHeader({ name, greeting, todayLabel, summary }: DashboardHeaderProps) {
  return (
    <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-blue-50 p-5 shadow-[0_20px_45px_rgba(15,23,42,0.06)] dark:border-slate-800 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950 sm:p-6 lg:p-7">
      <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <QuickActionButton href="/assignments/new" icon={<Plus className="h-4 w-4" />} label="New Assignment" />
          <QuickActionButton href="/notes/new" icon={<NotebookPen className="h-4 w-4" />} label="New Note" variant="secondary" />
          <QuickActionButton href="/courses/new" icon={<GraduationCap className="h-4 w-4" />} label="New Course" variant="secondary" />
        </div>
      </div>
