...
export function DashboardPage({ name, assignmentSummary, upcomingAssignments, recentNotes, todaySchedule, financeSummary, recentExpenses, currency = DEFAULT_CURRENCY }: DashboardPageProps) {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        {[...statCards.map((card) => card.label === "Pending Assignments" ? { ...card, value: String(assignmentSummary.pending), description: `${assignmentSummary.pending === 1 ? "1 assignment" : `${assignmentSummary.pending} assignments`} still in progress`, trend: "Live from PostgreSQL" } : card.label === "Monthly Expenses" ? { ...card, value: formatMoney(financeSummary.expenses), description: "This month", trend: "Live from PostgreSQL" } : card), {
        ...
        {[{ label: "Monthly Balance", value: formatMoney(financeSummary.income - financeSummary.expenses), description: "Income minus expenses", trend: "Live from PostgreSQL", icon: "Wallet", tone: "blue" as const }].map((card) => { const Icon = getIcon(card.icon); return <StatCard key={card.label} label={card.label} value={card.value} description={card.description} trend={card.trend} icon={<Icon className="h-5 w-5" />} tone={card.tone} />; })}
      </section>
